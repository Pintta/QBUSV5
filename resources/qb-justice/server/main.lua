...
QBCore.Commands.Add('setlawyer', 'Set someone as a lawyer', {{name = 'id', help = 'ID'}}, true, function(source, args)
...
QBCore.Commands.Add('removelawyer', 'Remove someone as lawyer', {{name = 'id', help = 'ID'}}, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    local playerId = tonumber(args[1])
    local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
    if Player.PlayerData.job.name == 'lawyer' then
